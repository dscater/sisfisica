...
class UserController extends Controller
{
    ...
    public function create()
    {
        ...
        $array_carreras[''] = 'Seleccione...';
        foreach ($carreras as $value) {
            $array_carreras[$value->id] = $value->nombre;
        }

        $array_paralelos[''] = 'Seleccione...';
        foreach ($paralelos as $value) {
            $array_paralelos[$value->id] = $value->nombre;
        }
        return view('users.create', compact('array_carreras', 'array_paralelos'));
    }

    public function store(UserStoreRequest $request)
    {
        $request['fecha_registro'] = date('Y-m-d');
        $usuario = new DatosUsuario(array_map('mb_strtoupper', $request->except('carrera_id', 'paralelo_id')));
        ...
        if ($request->tipo != 'ADMINISTRADOR') {
            ...
            $nuevo_usuario->paralelo_id = $request->paralelo_id;
        }
    ...
    public function edit(DatosUsuario $usuario)
    {
        ...
        return view('users.edit', compact('usuario', 'array_carreras', 'array_paralelos'));
    }
    ...
    public function update(DatosUsuario $usuario, UserUpdateRequest $request)
    {
        ...
        $usuario->user->name = mb_strtolower($request->email);
        $usuario->user->tipo = $request->tipo;
        if ($request->tipo != 'ADMINISTRADOR') {
            $usuario->user->carrera_id = $request->carrera_id;
            $usuario->user->paralelo_id = $request->paralelo_id;
        }
        if ($request->hasFile('foto')) {
            // antiguo
